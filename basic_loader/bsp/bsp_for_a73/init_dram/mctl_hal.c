...
//set AC PDR/PDD
	...
	{
		for(i=0;i<9;i++)//for dqs/dqs#,odt always on
		{	//byte 0
			reg_val = mctl_read_w(DATX0IOCR(i));
			reg_val &= ~(0x3U<<24);
			reg_val |= (0x2U<<24);
...
//zq calibration/training
	...
	//set zq para   DX2/3
	reg_val = mctl_read_w(ZQnPR(2));
	reg_val &= ~(0x000000ff);
	reg_val |= ( ((para->dram_zq)>>16 ) & 0xff);
	mctl_write_w(reg_val,ZQnPR(2));
...
signed int init_DRAM(int type, __dram_para_t *para)
{
	...
	UART_printf2("DRAM DRIVE INFO: V0.2\n");
