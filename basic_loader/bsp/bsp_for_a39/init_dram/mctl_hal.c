...
#define AW1639_IC_ID_REG		(0x3)
#define AW1639_DCDC5_VOL_CTRL	(0x25)
...
int set_cpu_voltage(void)
{
    ...
    sunxi_rsb_init(0);
	ret = sunxi_rsb_read(0, AW1639_IC_ID_REG, &pmu_id, 1);
	if (ret) {
		dram_dbg("Get pmu ID failed\n");
		return -1;
	}
	dram_dbg("pmu id=%x\n", pmu_id);
	//ret = rsb_write(AW1639_DCDC3_VOL_CTRL, &cpu_reg);
	//	ret = rsb_write(AW1639_DCDC2_VOL_CTRL, (1260-600)/20);
	//	ret = rsb_write(AW1639_DCDC3_VOL_CTRL, (1260-600)/20);
	//ret = rsb_write(AW1639_DCDC4_VOL_CTRL, (1200-600)/20);
	vol = (1500-1000)/50;
		ret = sunxi_rsb_write(0, AW1639_DCDC5_VOL_CTRL, &vol, 1);
...
signed int init_DRAM(int type, __dram_para_t *para)
{
    ...
    if(para->dram_type == 3)	//DDR3 DRAM_VCC=1.5V
    	set_cpu_voltage();
    //else LPDDR2/3 DRAM_VCC=1.2V
