...
int sunxi_charge_restart(int next_mode)
{
        if(!next_mode)
        {
                next_mode = PMU_PRE_BOOT_MODE; 
        }
        printf("set next mode %d\n", next_mode);
        axp_set_next_poweron_status(next_mode);
        board_display_set_exit_mode(0);
...
void sunxi_update_subsequent_processing(int next_work)
{
	...
	switch(next_work)
	{
		case SUNXI_UPDATE_NEXT_ACTION_CHARGE:	//enter charge in android!
			printf("reset to charge in android!!!");
			sunxi_charge_restart(0);
			break;
