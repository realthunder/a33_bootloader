...
extern  uint burn_private_start, burn_private_len;
/*
*******************************************************************************
...
static int __usb_get_descriptor(struct usb_device_request *req, uchar *buffer)
{
	...
	switch(req->wValue >> 8)
	{
		...
		case USB_DT_CONFIG:		//配置描述符
		{
			...
			inter_dscrptr->bInterfaceClass     = USB_CLASS_MASS_STORAGE;		//pburn storage
		...
		case USB_DT_STRING:
		{
			...
			if(string_index == 0)
			{
				...
				sunxi_udc_send_setup(bLength, (void *)sunxi_usb_pburn_dev[0]);
			}
			else if(string_index < SUNXI_USB_PBURN_DEV_MAX)
			{
				/* Size of string in chars */
				unsigned char i = 0;
				unsigned char str_length = strlen ((const char *)sunxi_usb_pburn_dev[string_index]);
				...
				{
					buffer[2+ 2*i + 0] = sunxi_usb_pburn_dev[string_index][i];
					buffer[2+ 2*i + 1] = 0;
				}
		...
		case SUNXI_USB_PBURN_SETUP:
			...
			switch(cbw->CBWCDB[0])
	  		{
	  			...
	  			case SCSI_REQ_SENSE:
					...
					trans_data.act_send_buffer = (uint)pburn_RequestSense;
				...
				case SCSI_VERIFY:
	  			...
	  			case SCSI_INQUIRY:
					...
					trans_data.act_send_buffer = (uint)pburn_InquiryData;
				...
				case SCSI_READ10:		//HOST READ, not this device read
	  				...
	  				sunxi_usb_dbg("asked size 0x%x\n", cbw->dCBWDataTransferLength);
					{
						...
						sunxi_usb_dbg("read sectors 0x%x\n", sectors);
				...
				case SCSI_WRITE10:		//HOST WRITE, not this device write
					...
					pburn_flash_start   = (cbw->CBWCDB[2]<<24) | cbw->CBWCDB[3]<<16 | cbw->CBWCDB[4]<<8 | cbw->CBWCDB[5]<<0;
					pburn_flash_sectors = (cbw->CBWCDB[7]<<8) | cbw->CBWCDB[8];
					sunxi_usb_dbg("command write start: 0x%x, sectors 0x%x\n", pburn_flash_start, pburn_flash_sectors);

					trans_data.recv_size 	   = min(cbw->dCBWDataTransferLength, pburn_flash_sectors * 512);
					trans_data.act_recv_buffer = (uint)trans_data.base_recv_buffer;

					pburn_flash_start += sunxi_partition_get_offset(0);
	  			...
	  			case 0xf3:			//自定义命令，用于烧录用户数据
	  				...
	  				switch(cbw->CBWCDB[1])
	  				{
						...
						case 3:             //小机端发送数据
						{
							...
							ret = sunxi_sprite_read(start + offset, sectors, trans_data.base_send_buffer);
							if(!ret)
							{
	  	...
	  	case SUNXI_USB_PBURN_RECEIVE_DATA:
			...
			if(sunxi_usb_pburn_write_enable == 1)
			{
				...
				ret = sunxi_sprite_write(pburn_flash_start, (trans_data.recv_size+511)/512, (void *)trans_data.act_recv_buffer);
				if(!ret)
				{
