...

			{
				...
				else  //如果是量产工具，则根据配置处理
				{
					if(!fes_work->next_mode)
					{
						...
						if(6 == sunxi_efex_next_action)
						{
							sunxi_efex_next_action = SUNXI_UPDATE_NEXT_ACTION_CHARGE;
							printf("enter android charge mode,sunxi_efex_next_action=%d\n", sunxi_efex_next_action);	
						}
					}
					else
					{
					...
					if((sunxi_efex_next_action <= SUNXI_UPDATE_NEXT_ACTION_NORMAL) || (sunxi_efex_next_action > SUNXI_UPDATE_NEXT_ACTION_CHARGE))
					{
						sunxi_efex_next_action = SUNXI_UPDATE_NEXT_ACTION_NORMAL;
						trans_data.app_next_status   = SUNXI_USB_EFEX_APPS_STATUS;
					}
