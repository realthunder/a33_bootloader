...
__s32 dsi_open(__u32 sel,disp_panel_para * panel)
{
	...
	} else if(panel->lcd_dsi_if == LCD_DSI_IF_COMMAND_MODE) {
...
__s32 dsi_dcs_wr(__u32 sel,__u8 cmd,__u8* para_p,__u32 para_num)
{
	...
	{
		__u32 i=0;
		dsi_dev[sel]->gen_pld_data.dwval = *(para_p+i+2) << 24 | *(para_p+i+1) << 16 | *(para_p+i+0) <<  8 | cmd << 0;
		dsi_delay_us(20);
		i+=3;
		while(i<para_num)
		{
			dsi_dev[sel]->gen_pld_data.dwval = *(para_p+i+3) << 24 |
											   *(para_p+i+2) << 16 |
											   *(para_p+i+1) <<  8 |
											   *(para_p+i+0) <<  0 ;
			i+=4;
	...
	dsi_dev[sel]->gen_hdr.dwval = (data1<<16 | data0<<8 | vc<<6 | dt);

	return 0;
}
*/

 __s32 dsi_dcs_wr(__u32 sel,__u32 cmd,__u8* para_p,__u32 para_num)
{
	__u8 dt,data0,data1;
	__u8 vc = 0;
	...
	/*
	dsi_dev[sel]->cmd_pkt_status.bits.gen_cmd_empty   = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.gen_cmd_full    = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.gen_pld_w_empty = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.gen_pld_w_full  = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.gen_pld_r_empty = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.gen_pld_r_full  = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.gen_rd_cmd_busy = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.res0            = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.dbi_cmd_empty   = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.dbi_cmd_full    = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.dbi_pld_w_empty = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.dbi_pld_w_full  = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.dbi_pld_r_empty = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.dbi_pld_r_full  = 0;    //?
	dsi_dev[sel]->cmd_pkt_status.bits.dbi_rd_cmd_busy = 0;    //?
	*/
	return 0;
}
...
//add function
__s32 dsi_gen_wr(__u32 sel,__u32 cmd,__u8* para_p,__u32 para_num)
{
	__u8 dt,data0,data1;
	switch(para_num)
	{
